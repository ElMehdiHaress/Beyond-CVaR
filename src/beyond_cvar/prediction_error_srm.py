...
max_trials = 10 ** 4  # number of trials
# loss norm
norm = '2'
...
for norm in ['1', '2']:
    ...
    for dist in distributions:
        ...
        rgd_cat = \
            stagewise_rgd_cat(initial_w, X, Y, delta, beta, max_iterations, max_gradient_descents, bandwidth, phi, dphi,
                              norm)[-1]
        empirical_minimization = erm(initial_w, X, Y, eta, iterations, phi, norm)[-1]
        rgd_mom = \
            stagewise_rgd_mom(initial_w, X, Y, delta, beta, max_iterations, max_gradient_descents, bandwidth, phi, dphi,
                              norm)[-1]
        ...
        if dist == 'normal':
            ...
            # ax.legend(loc='upper right', shadow=True)
        # plt.show()
        plt.savefig('/Users/omar/Desktop/Japan/Spectral risk measures/L' + norm + '_SRM_prediction_' + dist +
                    '(ave).pdf', )
